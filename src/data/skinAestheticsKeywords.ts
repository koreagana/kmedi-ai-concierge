...
export const SKIN_AESTHETICS_KEYWORDS: SkinAestheticsKeyword[] = [
  ...
  },
  {
    ...
    note: {
      zh: '痤疮治疗不仅限于减轻当前已有的炎症，还可根据皮肤状态，从皮脂调节、红色痘印、色素沉着到凹陷性疤痕，分阶段进行综合性的整体管理。',
      ko: '여드름 치료는 현재 올라온 염증을 줄이는 것뿐 아니라 피지 조절, 붉은 자국, 색소침착, 패인 흉터까지 피부 상태에 따라 단계적으로 접근할 수 있습니다.',
      en: "Acne treatment isn't limited to reducing current inflammation — depending on skin condition, it can also be approached in stages, addressing sebum control, red marks, pigmentation, and pitted scars comprehensively.",
      ar: 'لا يقتصر علاج حب الشباب على تقليل الالتهاب الحالي فقط، بل يمكن أيضاً اتباع نهج مرحلي شامل يشمل ضبط إفراز الزهم والآثار الحمراء والتصبغ وصولاً إلى الندبات الغائرة، بناءً على حالة البشرة.',
    },

...
<div class="layout">
  ...
  <div class="panel form-panel">
    <h2>医院 / 预约信息</h2>
    <div class="field">
      <label>병원명 (한국어) <span style="font-weight:400;color:var(--muted)">— 파트너 병원명을 입력하면 아래 항목이 자동으로 채워집니다</span></label>
      <input id="f-hospital-ko" value="서래셀의원" list="hospital-list" autocomplete="off">
      ...
      <div id="card">
        ...
        <div class="stub-addr">
          ...
          <div class="addr-row">
            ...
            <div class="addr-input" id="c-phone" contenteditable="true">-</div>
          </div>
...
/* 병원명(한국어) datalist 채우기 + 정확히 일치하는 병원 선택 시 中文名/영문명/주소/전화번호 자동 채움 */
...
function handleHospitalInput(){
  const typed = document.getElementById('f-hospital-ko').value.trim()
  const match = HOSPITALS.find(h => h.ko === typed)
  if (match) applyHospital(match)
  update()
}
...
document.getElementById('f-hospital-ko').addEventListener('input', handleHospitalInput);
